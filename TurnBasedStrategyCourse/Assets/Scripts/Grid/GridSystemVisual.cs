...
// This spawns the visual indicator prefab for valid move positions on the Grid
public class GridSystemVisual : MonoBehaviour
{
    ...
    public void HideAllGridPositionVisual()
    {
        for(int x = 0; x < LevelGrid.Instance.GetWidth(); x++)
        {
            for(int z = 0; z < LevelGrid.Instance.GetHeight(); z++)
    ...
    private void ShowGridPositionRange(GridPosition gridPosition, int range, GridVisualType gridVisualType)
    {
        ...
        for(int x = -range; x <= range; x++)
        {
            ...
            {
                // Gets the test grid position that is x and z grid positions away from the original grid position
                GridPosition testGridPosition = gridPosition + new GridPosition(x, z);
                // Ignores the grid positions that are not valid
                if(!LevelGrid.Instance.isValidGridPosition(testGridPosition))
                {
                    continue;
                }

                // Skips grid Positions outside the shooting range
    ...
    private void UpdateGridVisual()
    {
        ...
        // Shows all valid grid positions
